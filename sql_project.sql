...
#a.How many unique employees are currently in the system?
 ...
 select jobdept,count(jobdept) as dp from jobdepartment
 group by jobdept
 order by dp desc;
 
 #c.What is the average salary per department?
 select j.jobdept,
 round(avg(s.amount),2) as avg_amount_per_month,
 round(avg(s.annual),2)  as avg_amount_per_year
 from employee e
...
SET salaryrange = REPLACE(salaryrange, ' ', '');
UPDATE JobDepartment
SET min_salary = CAST(SUBSTRING_INDEX(salaryrange, '-', 1) AS UNSIGNED),
    max_salary = CAST(SUBSTRING_INDEX(salaryrange, '-', -1) AS UNSIGNED);
...
#a How many employees have at least one qualification listed?
select count( Requirements  is not null) as greater_1_qual from qualification;
...
#d● What is the total number of leave days taken by all employees?
select count(l.leave_ID) as total_leaves
from employee e
inner join leaves l
on l.emp_ID=e.emp_ID
order by total_leaves desc;
...
sum(total_amount) as total_payroll_to_employees from payroll;
